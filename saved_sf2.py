...
def myPSNR(smri,recon):
    ...
    mse=np.mean(np.square(np.abs(smri-recon)))
...
def genSenseMap(image_coil, crop_smap):
    ...
    out = np.squeeze(smap * np.expand_dims(mask, 3))
    import matplotlib.pyplot as plt
    plt.clf()
    for c in range(0, n_channels, 2):
        plt.subplot(4, 4, c // 2 + 1)
        plt.imshow(np.abs(out[:, :, 45, c]))
    plt.show()
    return out
...
def getFmriDataOnly(acq_f, requested_timepoints):
    ...
    return c2r(fmri_ifft) / 300000 #rough guestimate
...
def getData(smri_filename, acq_filename, requested_timepoints, do_inv, prev_mb_slice_matrix=None, sense_lam=0.1):
    ...
    if prev_mb_slice_matrix is not None:
        assert mb_slice_matrix == prev_mb_slice_matrix
    smri = np.rot90(smri, 2, axes=(2, 3)) / smri.max() # x,y dimensions
    ...
    smap = genSenseMap(craw_complex[:], 0.1)
    ...
    #print("max matrix norm is", np.linalg.norm(smaps, axis=(-2, -1)).max())
    out_fmri = None
    out_inv = None
    out_encode = None
